...
export default function ManufacturersPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await fetchApi('/api/manufacturers');
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editingItem) {
        await fetchApi(`/api/manufacturers/${editingItem.id}`, {
          method: 'PUT',
          body: JSON.stringify(data)
        });
        if (editingItem.status !== data.status) {
            await fetchApi(`/api/manufacturers/${editingItem.id}/status`, {
                method: 'PATCH',
                body: JSON.stringify({ status: data.status })
            });
        }
      } else {
        await fetchApi('/api/manufacturers', {
          method: 'POST',
          body: JSON.stringify(data)
        });
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetchApi(`/api/manufacturers/${id}`, { method: 'DELETE' });
      loadData();
    } catch (err: any) {
      setError(err.message);
    }
  ...
  if (loading) return <div className="center-screen text-gradient">Loading manufacturers...</div>;
    ...
    <div className="container" style={{ paddingTop: '4rem', paddingBottom: '4rem', maxWidth: '1000px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <div>
          ...
          <h1 className="text-gradient">Manufacturers</h1>
        </div>
        <Button onClick={() => handleOpenModal()}>+ Add Manufacturer</Button>
      </header>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {manufacturers.length === 0 ? (
          <p style={{ color: 'var(--text-secondary)' }}>No manufacturers found. Create one to get started.</p>
        ) : (
          manufacturers.map(s => (
            <Card key={s.id}>
          ...
          <Card style={{ width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', margin: '0 1rem', background: 'var(--bg-panel)', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            <CardHeader style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.25rem', color: 'var(--text-primary)' }}>
                {editingItem ? 'Edit Manufacturer' : 'New Manufacturer'}
              </h3>
            </CardHeader>
            <CardBody>
              ...
              {editingItem && (
                <div style={{ marginTop: '2rem' }}>
                  <MediaGallery entityType="manufacturer" entityId={editingItem.id} />
                </div>
              )}
            </CardBody>
          </Card>
