...
export default function ChannelsPage() {
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editingItem) {
        await fetchApi(`/api/channels/${editingItem.id}`, {
          method: 'PUT',
          body: JSON.stringify(data)
        });
        if (editingItem.status !== data.status) {
            await fetchApi(`/api/channels/${editingItem.id}/status`, {
                method: 'PATCH',
  ...
  if (loading) return <div className="center-screen text-gradient">Loading channels...</div>;
    ...
    <div className="container" style={{ paddingTop: '4rem', paddingBottom: '4rem', maxWidth: '1000px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <div>
          ...
          <h1 className="text-gradient">Channels</h1>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {channels.length === 0 ? (
          <p style={{ color: 'var(--text-secondary)' }}>No channels found. Create one to get started.</p>
        ) : (
          channels.map(s => (
            <Card key={s.id}>
            ...
            <CardBody>
              ...
              {editingItem && (
                <div style={{ marginTop: '2rem' }}>
                  <MediaGallery entityType="channel" entityId={editingItem.id} />
                </div>
              )}
            </CardBody>
          </Card>
