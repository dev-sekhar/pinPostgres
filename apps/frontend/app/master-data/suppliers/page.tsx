...
export default function SuppliersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await fetchApi(`/api/suppliers/${editingItem.id}`, {
          method: 'PUT',
  ...
  if (loading) return <div className="center-screen text-gradient">Loading suppliers...</div>;
    ...
    <div className="container" style={{ paddingTop: '4rem', paddingBottom: '4rem', maxWidth: '1000px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <div>
          ...
          <h1 className="text-gradient">Suppliers</h1>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {suppliers.length === 0 ? (
          <p style={{ color: 'var(--text-secondary)' }}>No suppliers found. Create one to get started.</p>
        ) : (
          suppliers.map(s => (
            <Card key={s.id}>
